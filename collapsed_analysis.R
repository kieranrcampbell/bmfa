...
g <- mfa_gibbs_constr(t(X), iter = 20000, thin = 10,
                      tau_k = 1, tau_c = 1, collapse = TRUE)
...
ggs_traceplot(filter(mc, Parameter == "eta[1]")) + stat_smooth()
ggs_traceplot(filter(mc, Parameter == "eta[2]")) + stat_smooth()
